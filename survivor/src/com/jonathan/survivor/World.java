...
public class World
{
	...
	private void updateLevelObjects(float deltaTime)
	{
		...
		//Stores true if there exists a zombie that is alert. If true, the zombie alert music plays.
		boolean zombieAlert = false;
		...
		for(int i = 0; i < gameObjects.size; i++)
		{
			...
			{
				//Stores the zombie that needs to be updated
				Zombie zombie = (Zombie) go;

				//Update the zombie's game logic.
				zombieManager.update(zombie, deltaTime);
				
				//If the zombie that is being updated is alert
				if(zombie.isAlerted())
					//Sets 'zombieAlert' to true, making the "Zombie Alert Music" play.
					zombieAlert = true;
			}
		...
		//If the player is in exploration state, choose which music to play
		/*if(getWorldState() == WorldState.EXPLORING)
		{
			//If there exists a zombie in the world that is alerted of the player's presence
			if(zombieAlert)
			{
				//Play the 'Zombie Alert Music' to tell the player that a zombie is aware of his presence.
				playSound(Sound.ZOMBIE_ALERT_MUSIC);
			}
			//Else, if no zombies in the world are alert
			else
			{
	...
	public void playVersusAnimation()
	{
		...
		//Start playing the enter combat music.
		playSound(Sound.ENTER_COMBAT_MUSIC);
	...
	public void enterCombat() 
	{
		...
		player.getZombieToFight().setState(State.ENTER_COMBAT);
	...
	{
		//Returns true if the GameObject is out of bounds of the TerrainLevel
		return level.outOfBounds(gameObject);
	}
	...
	public void spawnEarthquake(Zombie zombie) 
	{
		...
		//Play the sound of the earthquake spawning
		playSound(Sound.EARTHQUAKE);
	}
